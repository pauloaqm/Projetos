...
public class hud : MonoBehaviour {
	...
	void OnGUI () {
		...
		GUI.Label (new Rect (Screen.width-85,25,30,50),"x"+GetComponent<KitControllerBasico>().moedas.ToString(), moedaStyle);

		if (GetComponent<KitControllerBasico>().life/50 == 2) {
		...
		}else if (GetComponent<KitControllerBasico>().life/50 == 1) {
			GUI.DrawTexture(new Rect(30,10,40,40), lifeCheio, ScaleMode.ScaleToFit, true, 0F);
